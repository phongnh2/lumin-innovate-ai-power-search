...
export class DocumentTeamLevelPipeBase extends DocumentOrganizationLevelPipeBase implements PipeTransform<unknown> {
  constructor(
  ...
  async transform(value: unknown, argument: ArgumentMetadata): Promise<unknown> {
    ...
    const isVerifySuccess = await VerifyDocumentPermissionBase.Team({
      documentService: this.documentService,
      membershipService: this.membershipService,
      organizationService: this.organizationService,
      nextFunc: () => super.transform(value, argument),
      errorCallback: () => HttpErrorException.Unauthorized('You have no permission'),
      data: {
        requestData,
