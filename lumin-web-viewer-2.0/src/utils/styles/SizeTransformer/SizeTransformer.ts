...
export abstract class SizeTransformer<TSize> {
  ...
  protected formatSize(): IResponsiveSize<TSize> {
    ...
    Object.keys(castSize).forEach((resolution: SizeResolutionType) => {
      resolutionMap[resolution] = castSize[resolution];
    });

    let currentSize: TSize = null;
    let maxSize: TSize = null;
