...
export const getAngleInRadians = (pt1: Point, pt2: Point, pt3?: Point) => {
  let angle;

  if (pt1 && pt2) {
    if (pt3) {
      // calculate the angle using Law of cosines
      const AB = Math.sqrt((pt2.x - pt1.x) ** 2 + (pt2.y - pt1.y) ** 2);
      const BC = Math.sqrt((pt2.x - pt3.x) ** 2 + (pt2.y - pt3.y) ** 2);
      const AC = Math.sqrt((pt3.x - pt1.x) ** 2 + (pt3.y - pt1.y) ** 2);
