...
import useCreateOrganizationInPayment from 'hooks/useCreateOrganizationInPayment';
...
import { billingModalEvent } from 'utils/Factory/EventCollection/PaymentEventCollection';
...
import { useBillingFormContext } from './useBillingFormContext';
import { useTrialModalContext } from './useTrialModalContext';
...
export function useClaimFreeTrial({
  ...
  const { billingInfo, currentPaymentMethod, closeModal } = useTrialModalContext();
  const { getNewSecret } = useBillingFormContext();
  ...
  const { organization: selectedOrganization } =
    useSelector((state) => selectors.getOrganizationById(state as RootState, organizationId), shallowEqual) || {};
  const { payment: targetPayment, name } = selectedOrganization || { payment: {} as IOrganizationPayment };
  ...
  const trackUserFillPaymentForm = ({ fieldName, action }: { fieldName: string; action: string }) => {
    triggerEvent({
      callback: billingModalEvent.userFillPaymentForm.bind(billingModalEvent) as () => void,
  ...
  const logToPinpoint = (msg: string, cardInfo?: CardInfo) => {
    triggerEvent({
      callback: billingModalEvent.paymentError.bind(billingModalEvent) as () => void,
      params: {
        ...eventParams,
        ...cardInfo,
        errorMessage: msg,
  ...
  const getContentModal = () => ({
    ...
    message: (
      <Trans
        i18nKey="modalErrorSameTrial.message"
        values={{ name }}
  ...
  const getContentUpgradingInvoiceModal = (metadata: { plan: string; period: string; docStack: string }) => ({
    ...
    message:
      metadata.plan === Plans.ENTERPRISE ? (
        <Trans
          ...
          values={{ plan: PLAN_TYPE_LABEL[metadata.plan as keyof typeof PLAN_TYPE_LABEL] }}
        ...
        <Trans
          i18nKey="modalChangingPlan.message"
          values={{
            plan: PLAN_TYPE_LABEL[metadata.plan as keyof typeof PLAN_TYPE_LABEL],
  ...
  const handleError = (err: Error) => {
    ...
    const { code, message, stopped, metadata, isSetupIntentError } = errorUtils.extractGqlError(err) as {
      ...
      message: string;
      stopped: boolean;
      metadata: { plan: string; period: string; docStack: string };
  ...
  const trackEvent = ({
    ...
    triggerEvent({
      callback: billingModalEvent.paymentSuccess.bind(billingModalEvent) as () => void,
      params: {
        ...eventParams,
        ...cardInfo,
  ...
  const openSuccessModal = (organization: IOrganization) => {
  ...
  const handleChargeSuccess = ({
    newOrganization: _newOrganization,
    payment,
    cardInfo,
  }: {
  ...
  const onCompletePurchase = async (organization?: IOrganization) => {
    try {
  ...
  const beforePurchase = async (_e: React.MouseEvent<HTMLButtonElement>) => {
    if (isRestrictedOrg) {
    ...
    triggerEvent({
      callback: billingModalEvent.userSubmitPaymentForm.bind(billingModalEvent) as () => void,
      params: eventParams,
    });
