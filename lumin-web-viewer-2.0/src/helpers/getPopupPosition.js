...
const DEFAULT_GAP = 13;
const PAGE_NAVIGATION_HEIGHT = 60;

export const calcPopupLeft = ({ topLeft, bottomRight }, { width, height }, gap = DEFAULT_GAP) => {
  ...
  // we will place to right or left side if they have enough space.
  if (annotBottom + height > visibleRegion.bottom && annotTop - height < visibleRegion.top) {
    const annotLeft = topLeft.x - gap;
    const annotRight = bottomRight.x + gap;
    if (annotRight + width < visibleRegion.right) {
      return annotRight;
    }
    if (annotLeft - width > visibleRegion.left) {
      return annotLeft - width;
    }
  }
  /* END */
 ...
 */
export const calcPopupTop = ({ topLeft, bottomRight }, { height }, gap = DEFAULT_GAP) => {
  const padding = 5;
  const scrollContainer = core.getScrollViewElement();
  const boundingBox = scrollContainer.getBoundingClientRect();
  const visibleRegion = {
  ...
  if (top > visibleRegion.bottom - PAGE_NAVIGATION_HEIGHT - height) {
    top = visibleRegion.bottom - PAGE_NAVIGATION_HEIGHT - height;
  }

  return Math.round(top - scrollContainer.scrollTop);
};

const calcAnnotationPopupPosition = (annotationPosition, popupDimension, gap) => {
  const top = calcPopupTop(annotationPosition, popupDimension, gap);
  const left = calcPopupLeft(annotationPosition, popupDimension, gap);
...
const getSelectedTextPosition = (allQuads) => {
  ...
  const {
    ...
    right,
    bottom,
  } = getSelectedTextPageCoordinates(
    allQuads,
    startPageNumber,
    endPageNumber,
  );
...
export const getCommentPreviewPopupPositionBaseOn = (annotation, popup) => {
  const { left, top } = calcAnnotationPopupPosition(getAnnotationPosition(annotation), getPopupDimensions(popup), 0);
