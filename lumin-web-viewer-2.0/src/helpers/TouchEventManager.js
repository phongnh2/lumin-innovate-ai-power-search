...
const TouchEventManager = {
  ...
  isScrollingVertically() {
    return (
      (Math.abs(this.verticalMomentum) > 0 && this.horziontalMomentum === 0) ||
      Math.abs(this.touch.verticalDistance) > this.axisLockThreshold * Math.abs(this.touch.horizontalDistance)
    );
  },
  isScrollingHorziontally() {
    return (
      (Math.abs(this.horziontalMomentum) > 0 && this.verticalMomentum === 0) ||
      Math.abs(this.touch.horizontalDistance) > this.axisLockThreshold * Math.abs(this.touch.verticalDistance)
    );
  },
  canLockScrolling() {
  ...
  handleTouchEnd() {
    switch (this.touch.type) {
      ...
      case 'swipe': {
        ...
        const {
          reachedLeft, reachedTop, reachedRight, reachedBottom,
        } = this.reachedBoundary();
        ...
        const shouldGoToPrevPage =
          isSingleDisplayMode &&
          !isFirstPage &&
          ((swipedToLeft && this.allowHorizontalSwipe) || (swipedToTop && this.allowVerticalSwipe));
        const shouldGoToNextPage =
          isSingleDisplayMode &&
          !isLastPage &&
          ((swipedToRight && this.allowHorizontalSwipe) || (swipedToBottom && this.allowVerticalSwipe));
      ...
      case 'doubleTap': {
       // Will be update other action for this case
        break;
      }
      case 'pinch': {
  ...
  startMomentumScroll(touchDuration) {
    ...
    const momentumScroll = () => {
      ...
      const isMomentumScrollAlmostFinish =
        Math.abs(horzDiff) < momentumUnlockThreshold && Math.abs(vertDiff) < momentumUnlockThreshold;
  ...
  reachedBoundary() {
    const {
      clientHeight: scrollHeight, clientWidth: scrollWidth, scrollLeft, scrollTop,
    } = this.container;
  ...
  getPointAfterScale() {
    const x =
      (this.touch.clientX + this.container.scrollLeft - this.document.offsetLeft) * this.touch.scale -
      this.touch.clientX +
      this.container.offsetLeft;
    const y =
      (this.touch.clientY + this.container.scrollTop - this.document.offsetTop) * this.touch.scale -
      this.touch.clientY +
