...
export default (enable, arrayTools, store) => {
  let toolNameArray = arrayTools;
  if (!toolNameArray.length) {
    const toolNames = getAnnotationCreateToolNames();
    toolNameArray = typeof toolNames === 'string' ? [toolNames] : toolNames;
  }
  const map = {
