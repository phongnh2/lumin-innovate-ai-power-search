    ...
    return {
      ...
      [`${Keys.CTRL_V}, ${Keys.COMMAND_V}`]: (e) => {
        if (!isFocusingElement()) {
          ...
          const commentHighlightSelected = annotList.filter(
            (annot) => isComment(annot) && annot.getCustomData(CUSTOM_DATA_COMMENT_HIGHLIGHT.HIGHLIGHT_TEXT)
          );
          const annotationsCannotModify = annotList.filter((annot) => !core.canModify(annot));
  ...
  createToolHotkeyHandler(handler) {
    ...
    return (...args) => {
      ...
      const isSignatureModalOpen =
        currentToolName === window.Core.Tools.ToolNames.SIGNATURE &&
